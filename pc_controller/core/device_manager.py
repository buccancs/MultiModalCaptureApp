...
class DeviceManager(QObject):
    ...
    def _scan_wifi_networks(self):
        ...
        try:
            if platform.system() == "Windows":
                # Use netsh to scan for available WiFi networks
                result = subprocess.run([
    ...
    def _parse_windows_wifi_output(self, output: str):
        """Parse Windows netsh WiFi networks output."""
        try:
            lines = output.strip().split('\n')
            current_network = {}
            ...
            for line in lines:
                ...
                if line.startswith("SSID") and ":" in line:
                    # If we have a previous network, process it
                    if current_network and current_network.get("ssid"):
                        self._create_wifi_device_from_netsh_data(current_network)
                ...
                # Look for network type/authentication
            ...
            # Process the last network if exists
            if current_network and current_network.get("ssid"):
                self._create_wifi_device_from_netsh_data(current_network)
    ...
    def _create_wifi_device_from_netsh_data(self, network_data: dict):
        """Create WiFi device from parsed netsh network data."""
        ssid = network_data.get("ssid", "Unknown")
        bssid = network_data.get("bssid", "Unknown")
        
        if ssid and ssid != "Unknown":
            # Create security string from authentication and encryption
            auth = network_data.get("authentication", "Unknown")
            encryption = network_data.get("encryption", "Unknown")
            security = f"{auth}/{encryption}" if auth != "Unknown" and encryption != "Unknown" else "Unknown"
            
            # Extract signal strength number if available
            signal_str = network_data.get("signal", "")
            signal_strength = None
            if signal_str and "%" in signal_str:
                try:
                    # Extract percentage and convert to approximate dBm
                    percentage = int(signal_str.replace("%", "").strip())
                    # Rough conversion: 100% ≈ -30dBm, 0% ≈ -90dBm
                    signal_strength = -90 + (percentage * 0.6)
                except:
                    pass
            
            # Create frequency info from radio type and channel
            frequency = network_data.get("radio_type", "")
            channel = network_data.get("channel", "")
            if channel:
                frequency = f"{frequency} Ch{channel}" if frequency else f"Ch{channel}"
            ...
            device = WiFiDevice(
                ...
                bssid=bssid,
                security=security,
                signal_strength=int(signal_strength) if signal_strength else None,
